...
|---------|-------------|---------|
| `packages/extension` | Node.js | VS Code extension host - handles compilation, resolution, security |
| `packages/webview-app` | Browser | React 18 app rendered in VS Code webview iframe |
| `packages/shared` | Both | Shared TypeScript types and component registry |
...
```typescript title="packages/extension/security/TrustManager.ts"
...
```typescript title="packages/extension/security/CSP.ts"
...
sequenceDiagram
    ...
    Extension->>Registry: Register 12 services
...
1. **Service Registration** - Register 12 services in dependency order
...
4. **Commands** - Register all preview/zoom/config commands
...
> [!TIP]
> For detailed implementation guidelines, see [packages/extension/services/ARCHITECTURE.md](/packages/extension/services/ARCHITECTURE.md).
...
| **Examples** | TrustManager, ConfigManager, PreviewManager | ResolverSubsystem, MetaSubsystem |
...
- `MetaSubsystem` - Nextra `_meta.json` file watchers and cache

### Preview Management

The `PreviewManager` singleton manages all preview instances and webview panels.

```typescript title="packages/extension/preview/preview-manager.ts"
...
```typescript title="packages/extension/rpc-extension.ts"
interface WebviewRemoteHandle {
  // State management
  setTrustState(state: TrustState): void
  updatePreview(code: string, path: string, deps: string[]): void
...
```typescript title="packages/extension/rpc-extension-handle.ts"
class ExtensionHandle {
  ...
  // Document access
  getDocument(uri: string): Promise<string | undefined>
  openTextDocument(fsPath: string): Promise<void>
...
```typescript title="packages/extension/types/core/module-system.ts"
...
```typescript title="packages/extension/module-system/handlers/index.ts"
...
```typescript title="packages/extension/module-system/transform/transform.ts"
...
```typescript title="packages/extension/module-system/deps/import-extractor.ts"
// Fast pre-check regex
const IMPORT_PATTERN = /\b(import\s|require\s*\(|export\s+\*\s+from|export\s+\{[^}]*\}\s+from)\b/;
...
```typescript title="packages/webview-app/src/SafePreview.tsx"
...
```typescript title="packages/webview-app/src/TrustedPreview.tsx"
...
```typescript title="packages/webview-app/src/module-system/eval/evaluateModule.ts"
...
```typescript title="packages/webview-app/src/module-system/preload/shimLoader.ts"
// Docusaurus shims
...
```typescript title="packages/webview-app/src/rpc-webview.ts"
...
```typescript title="packages/extension/framework/FrameworkDetector.ts"
...
|-----------|-------------------|-----------------|
| Docusaurus | `@docusaurus/core` | `@theme/Tabs`, `@theme/Admonition` |
...
```typescript title="packages/extension/nextra/MetaResolver.ts"
...
```typescript title="packages/extension/compiler/plugins/loader.ts"
// Plugin spec formats
"plugin-name"                    // Just name
["plugin-name", { option: true }]  // With options
...
15+ themes from Markdown Preview Enhanced:
...
24 syntax highlighting themes via Shiki with CSS variables:

```typescript title="packages/extension/themes/types.ts"
type CodeBlockTheme =
...
The extension provides VS Code commands organized by category.
...
| `mdx-preview.commands.toggleScrollSync` | Toggle scroll synchronization |

### Cache Commands

| Command ID | Description |
|------------|-------------|
| `mdx-preview.commands.refreshModuleCache` | Clear resolver cache only (legacy) |
| `mdx-preview.commands.clearAllCaches` | Clear all caches (resolver, Sass, webview modules) |

The `clearAllCaches` command clears:
1. **Extension-side caches**: Resolver cache, Sass module cache
2. **Webview-side caches**: Module cache, style cache (via RPC)
...
| Command ID | Description |
|------------|-------------|
| `mdx-preview.commands.zoomIn` | Increase preview zoom |
| `mdx-preview.commands.zoomOut` | Decrease preview zoom |
...
| Webview | `vitest.config.ts` (webview-app) | jsdom | Test React components |
...
```typescript title="packages/extension/test/__mocks__/vscode.ts"
...
```typescript title="packages/extension/preview/evaluate-in-webview.ts"
try {
...
```typescript title="packages/webview-app/src/components/ErrorBoundary/ErrorBoundary.tsx"
...
```typescript title="packages/extension/security/checkFsPath.ts"
...
| `refreshModuleCache` | Clears resolver cache only (legacy) |
...
| `utils/` | Shared utilities (Semaphore, cache, file helpers) |

### Webview Package

| Directory | Purpose |
|-----------|---------|
| `src/App.tsx` | Root component, mode switching |
| `src/SafePreview.tsx` | Safe Mode rendering |
| `src/TrustedPreview.tsx` | Trusted Mode rendering |
| `src/module-system/` | Module caching, loading, evaluation |
| `src/rpc-webview.ts` | RPC communication, message queue |
| `src/context/` | React contexts for state |
| `src/hooks/` | Shared React hooks |
| `src/themes/` | Theme loading, application |

### Shared Package

| File | Purpose |
|------|---------|
| `index.ts` | Shared types (TrustState, FetchResult, etc.) |
| `registry/` | Component registry (generic, framework shims) |

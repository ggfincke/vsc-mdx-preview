...
3. Mode stage
   - trusted: `evaluateTrustedStage(...)`
   - safe: `evaluateSafeStage(...)`
...
Tailwind channel clearing/reset behavior is centralized in `tailwind-channel-utils.ts`.
...
function SafePreviewRenderer({ html }: { html: string }) {
  ...
  const { containerRef, handleImageClick, renderPortals, extractHeadings } =
    usePreviewSetup({ diagramMode: 'after-paint' });
...
| `rpc-direct-handlers.ts` | Immediate DOM/style/module-system handlers (non-queued)                         |
| `rpc-bootstrap.ts`       | Comlink endpoint bootstrap + `ExtensionRPC` wrap/expose handshake               |
| `webview-rpc-client.ts`  | Thin composition root wiring queue/registration/direct/bootstrap                |

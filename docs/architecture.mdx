...
| Resolver FS | resolver-factory.ts | Shared | 5s | Node resolution caching |
...
- **Babel** (~2MB) - Loaded on first transform, not activation
...
- File probing uses `Promise.all` for extension checks
- Dependency scanning reads files in parallel
- Module loading fetches dependencies in parallel with deduplication
- Tailwind scans dependencies in parallel

...
```typescript title="packages/shared/index.ts"
interface WebviewRPC {
  ...
  updatePreview(code: string, entryFilePath: string, entryFileDependencies: string[]): void
...
interface ExtensionRPC {
  ...
  openDocument(relativePath: string, line?: number, column?: number): Promise<void>
...
| Directory | Purpose |
|-----------|---------|
| `index.ts` | Shared types (TrustState, FetchResult, ExtensionRPC, WebviewRPC, etc.) |
| `registry/` | Component registry (generic, framework shims, query functions) |
| `logging/` | LogLevel, LogTags, TaggedLogger types, createTaggedLoggerFactory |
| `errors/` | ModuleError class, error codes, factory functions, suggestions |
| `config/` | SETTINGS_DEFAULTS, enums, JSON schema for VS Code settings |
| `utils/` | LRUCache, Semaphore, module ID utilities, validation helpers |
| `callout/` | Callout type definitions and normalizers |
| `icons/` | Centralized icon definitions (callout, GitHub alert, file tree) |
| `constants.ts` | Shared timing and limit constants |
| `core-modules.ts` | PRELOADED_MODULE_IDS list |
| `codegen/` | Code generation utilities (shims, config schema) |
| `shims/` | Shim configuration data |

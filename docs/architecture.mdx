...
- `CacheSubsystem` - Authoritative invalidator registry for resolver/security, transform, config, framework/Nextra, Tailwind/Sass, component, & icon-pack caches. `MDX: Clear All Caches` runs the registry without creating lazy services, then clears active webview module/style/dependency caches.
...
| `PreviewInitializer`     | Handshake plus preview-scoped config/Tailwind subscriptions  |
| `WatcherManager`         | Per-preview watcher and event-subscription disposal          |
...
`Preview.ts` delegates update routing to an explicit flow module and keeps
refresh orchestration inline:
...
- `Preview.refreshWebview()`
  - current-preview lookup through `PreviewManager`
...
4. Post-push artifacts: `postPushArtifacts(...)`
   - frontmatter theme override and complete Nextra meta snapshot (`null` outside Nextra or on resolution loss)
...
interface ExtensionRPC {
  ...
  reportPreviewSourceLine(line: number): Promise<PreviewSourceLineReportResult>; // 'accepted' | 'ignored' | 'retry'
...
sequenceDiagram
    ...
    Handler->>Handler: Extract structured dependencies from esmCode & transformedCode
...
interface ResolutionContext {
  ...
  dependencyKind?: ModuleDependencyKind; // import or require export condition
}
```
...
Executable dependencies retain their authored syntax and source order. Static ESM and literal dynamic imports use `es-module-lexer`; a lexical CommonJS scanner ignores comments, strings, regular expressions, and template raw text while descending into executable template expressions.
...
async function extractModuleDependencies(
  code: string
): Promise<ModuleDependency[]>;
```

> [!NOTE]
> TypeScript-only imports are removed before extraction. Computed dynamic imports remain native runtime expressions and are not prefetched; literal imports receive collision-safe runtime request identities so `import` and `require` can select distinct conditional package exports.
...
| Command ID                            | Description                                 |
| ------------------------------------- | ------------------------------------------- |
| `mdx-preview.commands.clearAllCaches` | Clear registered extension & webview caches |

The `clearAllCaches` command clears:

1. **Extension-side caches**: Resolution/path security, TypeScript & preview config, Babel/PostCSS, Sass/Tailwind, framework/Nextra metadata, component detection, & Mermaid icon packs
2. **Webview-side caches**: Module cache, style cache, dependency tracker (via RPC)

The extension cache registry checks service initialization state, so manual
clearing never creates an otherwise-unused lazy service.
...
| ---------------------- | ----------------------------------------------------------- |
| `package.json` change  | Resolver/Sass/framework invalidation + full preview refresh |
| Preview config change  | Nearest-config re-resolution + full preview refresh         |
| Tailwind input create  | Scoped detection invalidation + one debounced update        |
...
| Command          | Effect                                                                       |
| ---------------- | ---------------------------------------------------------------------------- |
| `clearAllCaches` | Runs every registered extension invalidator and clears active webview caches |

...
| `ImageHandler`  | `.png`, `.jpg`, `.svg`, `.gif`, `.webp`     | Convert to webview URIs                |
...
// Flush order (implicit in handler registration):
// 1. trust   - Must come first (sets rendering mode)
// 2. safe    - Content (mutually exclusive with trusted)
// 3. trusted
// 4. error   - Overlays
// 5. stale
```
...
const FRAMEWORK_RULES = [
  { id: 'docusaurus', packages: ['@docusaurus/core', '@docusaurus/preset-classic'] },
  { id: 'starlight',  packages: ['@astrojs/starlight'] },
  { id: 'nextra',     packages: ['nextra', 'nextra-theme-docs', 'nextra-theme-blog'] },
  { id: 'nextjs',     packages: ['next'], secondaryDeps: ['@next/mdx', 'next-mdx-remote', '@mdx-js/react'] },
  // fallback: 'generic'
];
```
...
23 syntax highlighting themes (21 themes plus `auto` and `default`) via Shiki with CSS variables:
...
The extension provides 14 VS Code commands organized by category.
...
tests/
├── helpers/
│   └── fixtures.ts      # Shared test fixtures & utilities

...
// consolidates duplicate image click handling from SafePreview & TrustedPreview
export function useImageLightbox() {
  const { openLightbox } = useLightbox();

  // handle image click to open lightbox
  // supports both native MouseEvent (for addEventListener) & React.MouseEvent (for onClick)

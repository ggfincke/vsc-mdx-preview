...
import { memo, useLayoutEffect, useState, useRef, ComponentType } from 'react';
...
// wrapped with React.memo to prevent re-renders when only zoom changes (content unchanged)
export const TrustedPreviewRenderer = memo(
  ...
  // Custom comparison: only re-render if content or evaluated component actually changed
  // Callbacks (onComponentReady, onError) are stable via useCallback in App.tsx
  (prevProps, nextProps) =>
    prevProps.content.code === nextProps.content.code &&
    prevProps.content.entryFilePath === nextProps.content.entryFilePath &&
    prevProps.evaluatedComponent === nextProps.evaluatedComponent
);

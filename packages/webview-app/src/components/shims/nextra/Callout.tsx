...
// Default icons for each type (GitHub-style icons as SVG)
const DEFAULT_ICONS: Record<Exclude<CalloutType, null>, ReactNode> = {
  default: (
    <svg
...
export function Callout({
  ...
  const icon = emoji ?? (type ? DEFAULT_ICONS[type] : null);

  // Build class names
  const typeClass = type ? `mdx-preview-nextra-callout-${type}` : '';
  const classes = ['mdx-preview-nextra-callout', typeClass, className]
    .filter(Boolean)
    .join(' ');

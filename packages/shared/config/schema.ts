// packages/shared/config/schema.ts
// canonical config schema for mdx-previewrc
...
export const MDX_PREVIEW_CONFIG_SCHEMA = {
  ...
  description:
    'Configuration file for MDX Preview extension (.mdx-previewrc.json)',
  type: 'object',
  properties: {
    remarkPlugins: {
      type: 'array',
    ...
    components: {
      type: 'object',
      description:
        'Map MDX component names to file paths. Only loaded in Trusted Mode.',
    ...
    framework: {
      ...
      enum: [...FRAMEWORK_IDS],
      description:
        'Framework override. Overrides auto-detection from package.json.',
      examples: ['docusaurus', 'nextjs', 'starlight'],
    },
    frameworkOptions: {
      type: 'object',
      description: 'Framework-specific options for MDX Preview.',
      properties: {
        enableShims: {
          type: 'boolean',
          default: true,
          description:
            'Enable framework component shims (e.g., @theme/Tabs for Docusaurus). Only works in Trusted Mode.',
        },
        customAliases: {
          type: 'object',
          description:
            'Custom import path aliases (e.g., { "@components": "./src/components" }).',
          additionalProperties: {
            type: 'string',
            description: 'Relative path the alias resolves to',
          },
          examples: [
            {
    ...
    tailwind: {
      type: 'object',
      description: 'Tailwind CSS options for MDX Preview.',
      properties: {
        enabled: {
          type: 'string',
        ...
        configPath: {
          type: 'string',
          description: 'Optional path to a Tailwind config file.',
        },
      },
      additionalProperties: false,
    },

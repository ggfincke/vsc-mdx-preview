...
/**
 * LRU cache for compiled Tailwind CSS with TTL-based expiration.
 *
 * Cache Behavior:
 * - **Max Entries**: 20 by default (CACHE_DEFAULT_MAX_ENTRIES), configurable via constructor or updateSettings()
 * - **TTL**: 5 minutes by default (CACHE_DEFAULT_TTL_MS), configurable via constructor or updateSettings()
 * - **Eviction**: LRU (least recently used) when exceeding max entries
 * - **Expiration**: Entries expire after TTL and are lazily removed on next access
 *
 * Cache Keys:
 * Generated by TailwindProcessor.buildCacheKey() as SHA1 hash of:
 * - Tailwind version ('v3' or 'v4')
 * - Extracted class content (space-separated list)
 * - Config file path + modification timestamp
 * - Entry CSS path + modification timestamp
 *
 * File timestamps in the cache key ensure automatic invalidation when config
 * or entry CSS files are modified, without requiring manual cache clearing.
 *
 * @see TailwindProcessor.buildCacheKey - Cache key generation logic
 * @see TailwindProcessor.process - Main usage point for this cache
 *
 * @example
 * ```typescript
 * const cache = new TailwindCache({ maxEntries: 50, ttlMs: 10 * 60 * 1000 });
 * cache.set(cacheKey, compiledCss);
 * const cached = cache.get(cacheKey); // null if expired or evicted
 * ```
 */
